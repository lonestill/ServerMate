  ...
  return (
    <div className="flex flex-col w-56 shrink-0 border-r border-[#2a2a35] bg-[#111116]">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                  ...
                  {!s.hasJar && (
                    <span className="text-[9px] text-[#55556a]">{t('no_jar')}</span>
                  )}
                  {s.autostart && !isRunning && (
                    <Power size={8} className="text-[#33334a]" title={t('autostart')} />
                  )}
                  {isRunning && (
                    <span className="text-[9px] text-[#1bd96a] font-semibold">{t('online_badge')}</span>
                  )}
                </div>
              </div>
            </button>
      ...
      {ctx && (
        ...
        >
          <CtxItem icon={FolderOpen} label={t('ctx_open_folder')} onClick={() => ctxAction(() => window.api.openServerFolder(ctx.server.name))} />
          <CtxItem icon={Pencil} label={t('ctx_rename')} onClick={() => ctxAction(() => onRenameServer(ctx.server))} />
          <div className="my-1 h-px bg-[#2a2a35]" />
          <CtxItem icon={Trash2} label={t('ctx_delete')} danger onClick={() => ctxAction(() => onDeleteServer(ctx.server))} />
        </div>
      )}
    </div>
  )
