...
export default function CrashReportsPanel({ server }) {
  ...
  const [selected, setSelected] = useState(null) // { filename, content }
      ...
      <div className="flex flex-col h-full">
        ...
        {summary?.description && (
          <div className="shrink-0 mx-4 mt-3 p-3 bg-red-500/8 border border-red-500/20 rounded-xl">
            <p className="text-[11px] font-semibold text-red-400 mb-0.5">Описание</p>
    ...
    <div className="flex flex-col h-full overflow-y-auto">
      <div className="p-5 max-w-lg flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h2 className="text-[14px] font-semibold text-[#ecedee]">Краш-репорты</h2>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-2 py-16">
            ...
            <p className="text-[13px] text-[#55556a]">Краш-репортов нет — хорошие новости</p>
            <p className="text-[11px] text-[#33334a]">Если сервер упадёт, отчёты появятся здесь</p>
          </div>
        ) : (
